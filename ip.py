...
# [{'ip': '192.168.56.1', 'mac': '00:00:00:00:00:00'}, {'ip': '169.254.229.202', 'mac': '00:00:00:00:00:00'}]

import netifaces # compatible avec netifaces2
import socket # gethostname()

# Récupérer le nom de la machine sur le réseau
...
def __add_ip(family_addresses:list, mac:str, liste:list, local:bool = False) -> bool :
...
def get_interfaces(localip:bool = False, IpV6:bool = False) -> list :
    ...
    for interface in interfaces :
        ...
        mac = addresses[netifaces.AF_LINK][0]['addr'] # AF_LINK dépend du système
        if IpV6 :
            for address_family in (netifaces.AF_INET, netifaces.AF_INET6) :
                family_addresses = addresses.get(address_family)
                __add_ip(family_addresses, mac, ips_list, localip)
        else :
...
if __name__ == '__main__' :
    ...
    print("***********************************")
    interfaces = get_interfaces(False, False)
    for interface in interfaces :
        print("IP =", interface['ip'], "MAC =", interface['mac'])
